...
jest.mock("./BoxPlotItem", () => {
  return ({ fillColor, setInteractionData }) => (
    <g data-testid="boxplot-item-mock">
      <rect
        data-testid={`boxplot-item-${fillColor}`}
        onClick={() => setInteractionData({ xPos: 1, yPos: 1, children: <span>x</span> })}
      />
    </g>
  );
});
